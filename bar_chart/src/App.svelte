<script>
  ...
  const orgs = rollups( // change from continents
    data,
    v => mean(v, d => d.TotalCost), //change from happiness
    d => d.Org    // change from continent
  ) 
  // group by org, return group means
    .sort((a, b) => a[1] - b[1]) 
    .map(d => d[0]); 
    
  // Color is based on Org
  const colorRange = [
    //"#eb4c42",
    '#f9b9ac',
    '#68b6a3',
  ...
  let colorScale = scaleOrdinal()
    .domain(orgs) //array of orgs
    .range(colorRange) 

  let xScale = scaleBand()   //band makes discrete bands for values on the x axis and includes padding to put space bt bars
    .domain(orgs) // domain is list of divisions we want to make
    .range([0,innerWidth])  // the domain is scaled to a range of values that returns the START val of each band
    .padding(0.5);

  let yScale = scaleLinear()
    .domain([0,14500]) // hard coded, range of totalCost
    .range([innerHeight, 0])
...
class='chart-container' 
  ...
  on:click = {() => {
    groupByOrg = !groupByOrg; // change bool value
    hovered = null;
  }}
  >
  <svg {width} {height}>
    ...
    {#if hovered} 
      <line
        ...
        y2 = {hovered.y + margin.top + radiusScale(hovered.TotalCost)}
        stroke = {colorScale(hovered.Org)}
        stroke-width = "1"
        />
    {/if}
    ...
    >
      ...
      {#each nodes as node, ind}
        ...
        <bar 
          ...
          fill = {colorScale(node.Org)} 
          title = {node.ExpenseProjID}
          
          opacity={hovered || hoveredOrg        // set opacity based on hover
            ? hovered === node || hoveredOrg === node.Org
              ? 1
              : 0.2
            : 0.9}
          
          stroke={hovered || hoveredOrg       // set stroke based on hover
            ? hovered === node || hoveredOrg === node.Org
              ? "black"
              : "transparent"
            : "#00000090"}
